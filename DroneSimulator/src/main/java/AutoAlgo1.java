...
public class AutoAlgo1 {
	...
	public AutoAlgo1(Map realMap) {
		...
		drone.addLidar(0);//forward
		drone.addLidar(90);//right
		drone.addLidar(-90);//left
		drone.addLidar(180);//back
		sensor_distance=new DataSensor(drone);
	...
	public void ai(int deltaTime) {
		...
		if(is_init) {
			speedUp();
			HomePoint = drone.getOpticalSensorLocation();
			init_point = new Point(HomePoint);
			points.add(HomePoint);
			mGraph.addVertex(HomePoint);
			is_init = false;
			drone.Takeoff();
		}
		Point dronePoint = drone.getOpticalSensorLocation();
		boolean check_battrey=drone.isBatteryEmpty();
		if(check_battrey) {
			drone.Land();
			speedDown();
			drone.stop();
		...
		if(!is_risky) {
			...
			if(sensor_distance.Forward_distance() <= max_risky_distance ) {
				is_risky = true;
				risky_dis = sensor_distance.Forward_distance();

			}



			if(sensor_distance.Right_distance() <= max_risky_distance/3 ) {
				is_risky = true;
			}


			if(sensor_distance.Left_distance() <= max_risky_distance/3 ) {
				is_risky = true;
			}
...
//	public String getInfo() {
	...
	//	String info = "<html>";
		...
		//info += "<span style='color: blue;'>Home Point: " + this.HomePoint +"&deg;</span><br>";
	//	info += "</html>";
